...
export default function Sidebar({ content }: { content: any }) {
  return (
    <aside className="w-72 rounded-t-xl md:rounded-none flex flex-none h-full bg-slate-100 overflow-hidden flex-col flex-nowrap dark:bg-slate-800">
      ...
      <ul className="sidebar w-full p-2.5 flex flex-col flex-nowrap overflow-y-auto gap-1 flex-1">
        {content.channels &&
          content.channels.map((channel: any, index: number) => {
            return (
              <SidebarItem key={index} type={channel.type}>
                {channel.title}
              </SidebarItem>
            );
          })}
